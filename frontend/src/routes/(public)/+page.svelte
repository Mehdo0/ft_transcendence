<script lang="ts">
	...
	let rejoinGame = $state<RejoinGame>({
		opponents: [],
		players: [] as string[],
		time_left: 0,
		countdown_left: 0,
	...
	function rejoin() {
		...
		sessionStorage.setItem('draw_round_number', String(rejoinGame.round_number));
		sessionStorage.setItem('draw_in_progress', '1');
		saveRoundTiming(rejoinGame.time_left, rejoinGame.countdown_left);
		...
		async function initialize() {
			...
			unsubscribe = subscribe((msg) => {
				...
				rejoinGame = {
					...
					countdown_left: msg.countdown_left ?? 0,
					is_ranked: msg.is_ranked ?? false,
					word: msg.word || '',
					scores: msg.scores || {},
					round_wins: msg.round_wins || {},
					round_number: msg.round_number ?? 1
				};
				showRejoin = true;
			});

			timeout = setTimeout(() => unsubscribe(), 3000);
			void connect().catch(() => unsubscribe());
		}
...
<div class="flex flex-1 flex-col items-center justify-center gap-12 py-8">
	...
	<main class="flex w-full max-w-[440px] flex-col gap-4">
		<a href={resolve('/start_game')} class="{tileBase} bg-primary text-on-primary">
			<span>Play Now!</span>
			<span
		...
		<a href={resolve('/lobby')} class="{tileBase} bg-accent text-ink">
		...
		<a href={resolve('/ranking')} class="{tileBase} bg-highlight text-on-primary">
