<script lang="ts">
    ...
    function clearSessionData() {
        ...
        sessionStorage.removeItem('draw_my_wins'); 
        sessionStorage.removeItem('draw_opp_wins'); 
    }

    function updateRoundWins(winsDict: Record<string, number>) {
        if (!winsDict) return;
        ...
        sessionStorage.setItem('draw_my_wins', myRoundWins.toString());
        sessionStorage.setItem('draw_opp_wins', opponentRoundWins.toString());
    }
    ...
    onMount(() => {
        ...
        const savedMyWins = sessionStorage.getItem('draw_my_wins');
        if (savedMyWins) myRoundWins = parseInt(savedMyWins);

        const savedOppWins = sessionStorage.getItem('draw_opp_wins');
        if (savedOppWins) opponentRoundWins = parseInt(savedOppWins);
        ...
        ws.onmessage = (event) => {
            ...
            switch (msg.type) {
                ...
                case 'reconnect_game':
                    ...
                    if (msg.round_wins) updateRoundWins(msg.round_wins);
                ...
                case 'next_round':
                    ...
                    sessionStorage.setItem('draw_opp_score', '0');
                    stack = [];
                    redoStack = [];
                    redraw();
