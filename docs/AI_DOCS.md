...
Because we don't have Google's process power, we won't be able to use the full 50 million drawings provided by them ... Although we will use 10 distinct categories out of the 345 available. 
...
Basically, we'll tell the AI to analyze the drawing in multiple steps like layers :
...
Finally, we'll save its brain and plug it into the server loop
